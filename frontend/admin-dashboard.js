...
async function loadLiveData() {
    try {
            ...
            if (res && res.success && res.data) {
                ...
                if (empList.length > 0) {
                    employees = empList.map(e => ({
                        ...
                        name: `${e.personalDetails?.firstName || ''} ${e.personalDetails?.lastName || ''}`.trim() || e.name || 'Employee',
                        dept: e.jobDetails?.department || 'Engineering',
                        desig: e.jobDetails?.designation || 'Staff',
                        type: e.jobDetails?.employmentType || 'Full-time',
                        joinDate: e.jobDetails?.joiningDate ? String(e.jobDetails.joiningDate).substring(0,10) : '2026-01-01',
        ...
        if (typeof getAllAttendance === 'function') {
            const res = await getAllAttendance();
            if (res && res.success && res.data) {
                const list = Array.isArray(res.data) ? res.data : (res.data.records || []);
                if (list.length > 0) {
                    attendance = list.map(a => ({
                        empId: a.employeeId || a.employee?.employeeId || '-',
                        name: a.employee?.personalDetails?.firstName ? `${a.employee.personalDetails.firstName} ${a.employee.personalDetails.lastName || ''}` : 'Employee',
        ...
        if (typeof getAllLeaves === 'function') {
            ...
            if (res && res.success && res.data) {
                ...
                if (list.length > 0) {
                    leaves = list.map((l, i) => ({
                        id: l._id || i,
                        emp: l.employeeId || 'Employee',
                        dept: 'Engineering',
                        type: l.leaveType ? l.leaveType.toUpperCase() : 'Leave',
                        start: l.startDate ? String(l.startDate).substring(0, 10) : '',
                        end: l.endDate ? String(l.endDate).substring(0, 10) : '',
                        days: l.totalDays || 1,
                        reason: l.reason || '-',
                        applied: l.createdAt ? String(l.createdAt).substring(0, 10) : '',
        ...
        if (typeof getAdminDashboardReport === 'function') {
            const res = await getAdminDashboardReport();
            if (res && res.success && res.data) {
                const d = res.data;
                if (d.employeeStats) {
                    const el1 = document.getElementById('statTotalEmp');
                    const el2 = document.getElementById('statActiveEmp');
                    if (el1) el1.innerText = d.employeeStats.totalEmployees || employees.length;
                    if (el2) el2.innerText = d.employeeStats.activeEmployees || employees.length;
                }
                if (d.todayAttendance) {
                    const el = document.getElementById('statPresent');
                    if (el) el.innerText = d.todayAttendance.present || 0;
                }
                if (d.leaveStats) {
                    const el = document.getElementById('statPendingLeaves');
                    if (el) el.innerText = d.leaveStats.pending || 0;
                }
            }
        }
    } catch (e) {
        console.log("Live data load status:", e.message);
    }
}
