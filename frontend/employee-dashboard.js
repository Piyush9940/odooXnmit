...
document.addEventListener("DOMContentLoaded", () => {
    ...
    initChart();
    initForms();
    loadEmployeeLiveData();
});

async function loadEmployeeLiveData() {
    try {
        if (typeof getMyProfile === 'function') {
            const res = await getMyProfile();
            if (res && res.success && res.data) {
                const u = res.data;
                mockUser.name = `${u.personalDetails?.firstName || ''} ${u.personalDetails?.lastName || ''}`.trim() || u.name || mockUser.name;
                mockUser.empId = u.employeeId || mockUser.empId;
                mockUser.email = u.user?.email || u.email || mockUser.email;
                mockUser.phone = u.personalDetails?.phone || mockUser.phone;
                mockUser.department = u.jobDetails?.department || mockUser.department;
                mockUser.designation = u.jobDetails?.designation || mockUser.designation;
                if (u.personalDetails?.address) {
                    const a = u.personalDetails.address;
                    mockUser.address = `${a.street || ''} ${a.city || ''} ${a.state || ''}`.trim() || mockUser.address;
                }
                populateProfile();
            }
        }
        ...
        if (typeof getMyLeaves === 'function') {
            const res = await getMyLeaves();
            if (res && res.success && res.data) {
                const list = Array.isArray(res.data) ? res.data : (res.data.records || []);
                if (list.length > 0) {
                    mockLeaveRequests = list.map(l => ({
                        type: l.leaveType ? l.leaveType.toUpperCase() + ' Leave' : 'Paid Leave',
                        start: l.startDate ? String(l.startDate).substring(0, 10) : '',
                        end: l.endDate ? String(l.endDate).substring(0, 10) : '',
                        days: l.totalDays || 1,
        ...
        if (typeof getMyNotifications === 'function') {
            ...
            if (res && res.success && res.data) {
                ...
                if (list.length > 0) {
                    mockNotifications = list.map((n, i) => ({
                        id: n._id || i,
                        title: n.title || 'Notification',
                        msg: n.message || '',
                        time: n.createdAt ? String(n.createdAt).substring(0, 10) : 'Recent',
                        icon: 'fa-bell',
                        read: !!n.read
                    }));
                    populateNotifications();
        ...
        console.log("Employee live data check:", e.message);
    }
}
...
function initForms() {
    ...
    btnToggleAtt.addEventListener("click", async () => {
        const now = new Date();
        ...
        if (!isCheckedIn) {
            try {
                if (typeof checkInAttendance === 'function') {
                    await checkInAttendance("office");
                }
            } catch (err) {
                console.log("Checkin API status:", err.message);
            }
            isCheckedIn = true;
            ...
            try {
                if (typeof checkOutAttendance === 'function') {
                    await checkOutAttendance();
                }
            } catch (err) {
                console.log("Checkout API status:", err.message);
            }
    ...
    document.getElementById("applyLeaveForm").addEventListener("submit", async (e) => {
        ...
        const reason = document.getElementById("leaveReason")?.value || "Personal";

        let mapType = "paid";
        if (type.toLowerCase().includes("sick")) mapType = "sick";
        if (type.toLowerCase().includes("unpaid")) mapType = "unpaid";

        try {
            if (typeof applyLeave === 'function') {
                await applyLeave({ leaveType: mapType, startDate: start, endDate: end, reason });
            }
        } catch (err) {
            console.log("Apply leave API:", err.message);
        }
        ...
        mockLeaveRequests.unshift({
            ...
            days: parseInt(durationInput.value) || 1,
            status: "Pending"
        });
    ...
    document.getElementById("editProfileForm")?.addEventListener("submit", async (e) => {
        ...
        try {
            if (typeof updateMyProfile === 'function') {
                await updateMyProfile({ personalDetails: { phone, address: { street: address } } });
            }
        } catch(e){}
